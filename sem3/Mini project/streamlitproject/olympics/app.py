...
if user_menu == 'Medal Prediction':
    ...
    st.sidebar.write("Enter athlete details to predict the medal:")
    
    age = st.sidebar.number_input("Age", min_value=0, max_value=80, value=25)
    sex = st.sidebar.selectbox("Sex", ['M', 'F'])
    height = st.sidebar.number_input("Height (cm)", min_value=0, max_value=275, value=175)
    weight = st.sidebar.number_input("Weight (lbs)", min_value=0, max_value=1000, value=70)
    region = st.sidebar.selectbox("Country", df['region'].unique())
    city = st.sidebar.selectbox("Olympics Host City", df['City'].unique())
    sport = st.sidebar.selectbox("Sport Discipline", df['Sport'].unique())

    if st.sidebar.button("Predict Medal"):
        # Call AthletePred function from athpred.py
        prediction_result = helper.AthletePred(df, age, sex, height, weight, region, city, sport)
        
        # Display the prediction result on the main side
        st.title("Medal Prediction Result")
        
        
        st.write(
    f"<div style='font-size: 60px;'>{prediction_result}</div>",
